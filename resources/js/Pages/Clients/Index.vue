...
const onPageClick = (event)=>{
    formPage.get(route('cients.index',{page:event}))
}
const openModal= (op,name,email,client)=>{
    modal.value=true;
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12">
            <div class="bg-white grid v-screen place-items-center">
                <div class="mt-3 mb-3 flex">holaaaaaaaaaaaaaaaaaaaaaa{{ $page.props.auth.user }}
            ...
            <div class="bg-white grid v-screen place-items-center overflow-x-auto">
                <table class="table-auto border border-gray-400">
                    ...
                    <tbody>
                        <tr v-for="cli, i in Users" :key="cli.id">
                            <td class="border border-gray-400 px-2 py-2">{{ (i+1) }}</td>
                            <td class="border border-gray-400 px-2 py-2">{{ cli.name }}</td>
                            <td class="border border-gray-400 px-2 py-2">{{ cli.email }}</td>
                            <td class="border border-gray-400 px-2 py-2">
                                <WarningButton @click="openModal(2,cou.name,cou.email,cou.id)">
